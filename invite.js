...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(new Discord.MessageEmbed().setColor("GREEN").setDescription(`**${member}** adlı kişi sunucuya katıldı.Davet eden **<@${invite.inviter.id}>** kişisinin \`${total + bonus}\` daveti oldu. `)
        )}
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(new Discord.MessageEmbed().setColor("RED").setDescription(`${member.user.tag} adlı kişi sunucudan ayrıldı.Davet eden <@${user.id}> kişisinin \`${Number(total) + Number(bonus)}\` daveti kaldı`)
    ) }
});
